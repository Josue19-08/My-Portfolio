...
function Band({maxSpeed = 50, minSpeed = 0, isMobile = false, cardTextureUrl}: BandProps) {
    ...
    useFrame((state, delta) => {
        if (dragged && typeof dragged !== 'boolean') {
            ...
            // Circular boundary: card can only move within radius from its rest point
            const RADIUS = 1.6;
            const REST_X = 0, REST_Y = -0.45;
            const rawX = vec.x - dragged.x;
            const rawY = vec.y - dragged.y;
            const dx = rawX - REST_X;
            const dy = rawY - REST_Y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            const clampedX = dist > RADIUS ? REST_X + (dx / dist) * RADIUS : rawX;
            const clampedY = dist > RADIUS ? REST_Y + (dy / dist) * RADIUS : rawY;
